...
namespace backend {
    ...
    int _winWidth, _winHeight, _winLeft, _winTop;
    GLFWwindow* window;
    GLFWmonitor* monitor;
    ...
    int init(std::string resDir) {
        ...
        winLeft = core::configManager.conf["windowPosition"]["left"];
        winTop = core::configManager.conf["windowPosition"]["top"];
        ...
        for (int i = 0; i < OPENGL_VERSION_COUNT; i++) {
            ...
            // Create window with graphics context
            monitor = glfwGetPrimaryMonitor();            
            window = glfwCreateWindow(winWidth, winHeight, "SDR++ - Spherea v" VERSION_STR " (Built at " __TIME__ ", " __DATE__ ")", NULL, NULL);
            if (window == NULL) {
            ...
            glfwSetWindowPos(window, winLeft, winTop);
            glfwMakeContextCurrent(window);
            break;
        }
        ...
        glfwGetWindowPos(window, &_winLeft, &_winTop);
        if (fullScreen) {
    ...
    int renderLoop() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if ((_winWidth != winWidth || _winHeight != winHeight || _winTop != winTop || _winLeft != winLeft) && !maximized && _winWidth > 0 && _winHeight > 0) {
