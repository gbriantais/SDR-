...
void SinkManager::Stream::init(dsp::stream<dsp::stereo_t>* in, EventHandler<float>* srChangeHandler, float sampleRate) {
    ...
    volumeAjust.init(&volumeInput, 1.0, 1.0, false, false);
...
void SinkManager::Stream::setVolume(float volume) {
    guiVolume = volume;
    volumeAjust.setLeftVolume(volume);
    volumeAjust.setRightVolume(volume);
}
...
void SinkManager::Stream::setLeftVolume(float volume) {
    leftVolume = volume;
    applyStereoSettings();
}

void SinkManager::Stream::setRightVolume(float volume) {
    rightVolume = volume;
    applyStereoSettings();
}

float SinkManager::Stream::getLeftVolume() {
    return leftVolume;
}

float SinkManager::Stream::getRightVolume() {
    return rightVolume;
}

void SinkManager::Stream::setLeftMuted(bool muted) {
    leftMuted = muted;
    applyStereoSettings();
}

void SinkManager::Stream::setRightMuted(bool muted) {
    rightMuted = muted;
    applyStereoSettings();
}

bool SinkManager::Stream::getLeftMuted() {
    return leftMuted;
}

bool SinkManager::Stream::getRightMuted() {
    return rightMuted;
}

void SinkManager::Stream::applyStereoSettings() {
    volumeAjust.setLeftVolume(leftMuted ? 0.0f : leftVolume);
    volumeAjust.setRightVolume(rightMuted ? 0.0f : rightVolume);
}
...
void SinkManager::registerStream(std::string name, SinkManager::Stream* stream) {
    ...
    showVolumeSliderMap[name] = true;

    showStereoControlsMap[name] = false; // Par défaut, masqués
...
void SinkManager::unregisterStream(std::string name) {
    ...
    showStereoControlsMap.erase(name);
    showVolumeSliderMap.erase(name);
...
void SinkManager::showVolumeSlider(std::string name, std::string prefix, float width, float btnHeight, int btnBorder, bool sameLine, bool showSlider) {
    ...
    // Vérifier si les contrôles stéréo sont activés
    bool useStereoControls = showStereoControlsMap.find(name) != showStereoControlsMap.end() && showStereoControlsMap[name];

    if (useStereoControls) {
        // Mode stéréo : afficher les boutons gauche et droit

        // Left channel mute/unmute button
        ImGui::PushID(ImGui::GetID(("sdrpp_left_mute_btn_" + name).c_str()));
        if (stream->getLeftMuted()) {
            if (ImGui::ImageButton(icons::LEFTMUTED, ImVec2(height, height), ImVec2(0, 0), ImVec2(1, 1), btnBorder, ImVec4(0, 0, 0, 0), ImGui::GetStyleColorVec4(ImGuiCol_Text))) {
                stream->setLeftMuted(false);
                core::configManager.acquire();
            ...
            if (ImGui::ImageButton(icons::LEFTUNMUTED, ImVec2(height, height), ImVec2(0, 0), ImVec2(1, 1), btnBorder, ImVec4(0, 0, 0, 0), ImGui::GetStyleColorVec4(ImGuiCol_Text))) {
                stream->setLeftMuted(true);
                core::configManager.acquire();
                saveStreamConfig(name);
                core::configManager.release(true);
            }
        ...
        // Right channel mute/unmute button
        ImGui::PushID(ImGui::GetID(("sdrpp_right_mute_btn_" + name).c_str()));
        if (stream->getRightMuted()) {
        ...
        // Mode normal : afficher le bouton principal mute/unmute
    ...
    // Contrôle de volume
    if (showVolumeSliderMap[name] && showSlider) {
        ImGui::SameLine();
        float buttonCount = useStereoControls ? 2.0f : 1.0f;
        float spacingBetweenButtons = useStereoControls ? (ImGui::GetStyle().ItemSpacing.x) : 0.0f;
        float totalButtonWidth = (buttonCount * height) + spacingBetweenButtons;
        float sliderWidth = width - totalButtonWidth - sliderOffset;
        sliderWidth = std::max<float>(sliderWidth, 50.0f * style::uiScale);
...
void SinkManager::loadStreamConfig(std::string name) {
    ...
    // Chargement de l'état des contrôles stéréo (avec valeur par défaut)
    if (conf.contains("showStereoControls")) {
        showStereoControlsMap[name] = conf["showStereoControls"];
    }
    else {
        showStereoControlsMap[name] = false; // Par défaut, masqués
    }
...
void SinkManager::saveStreamConfig(std::string name) {
    ...
    conf["showStereoControls"] = showStereoControlsMap[name]; // Sauvegarde de l'état des contrôles stéréo
...
void SinkManager::showMenu() {
    ...
    for (auto const& [name, stream] : streams) {
        ...
        // Case à cocher pour activer/désactiver les contrôles stéréo
        if (ImGui::Checkbox(("Adjust Right/Left volume##" + name).c_str(), &showStereoControlsMap[name])) {
            core::configManager.acquire();
            saveStreamConfig(name);
            core::configManager.release(true);
        }

        // Les contrôles left/right ne sont plus affichés ici, ils remplacent maintenant le bouton principal
