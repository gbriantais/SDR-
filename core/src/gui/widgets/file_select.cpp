...
static bool isRegularFileUtf8(const std::string& path) {
#ifdef _WIN32
    try {
...
bool FileSelect::render(std::string id) {
    ...
    if (ImGui::InputText(id.c_str(), strPath, 2047)) {
        ...
        if (!isRegularFileUtf8(expandedPath)) {
            pathValid = false;
        }
        else {
            pathValid = true;
...
void FileSelect::setPath(std::string path, bool markChanged) {
    ...
    pathValid = isRegularFileUtf8(expandedPath);
...
void FileSelect::worker() {
    ...
    if (pathValid) {
        try {
            startDir = std::filesystem::u8path(expandString(path))
                           .parent_path()
                           .string();
        }
        catch (...) {
    ...
    auto file = pfd::open_file(
        "Open File",
        pathValid ? std::filesystem::path(expandString(path)).parent_path().string() : "",
        _filter
    );
#endif
    ...
    pathValid = isRegularFileUtf8(expandString(path));
    dialogOpen = false;
}