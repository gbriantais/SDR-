...
void MainWindow::init() {
    ...
    // === Charger les limites de fréquence ===
    autoFreqLimitEnabled = core::configManager.conf["frequency_limits"]["enabled"];
    // Charger le zoom mémorisé (slider 0..1)
    float savedZoom = 1.0f;
    if (core::configManager.conf.contains("viewBandwidthSlider")) {
        savedZoom = core::configManager.conf["viewBandwidthSlider"];
    }

    // Charger la configuration autostart
    bool autoStartConf = false;
    if (core::configManager.conf.contains("autoStart")) {
        autoStartConf = core::configManager.conf["autoStart"];
    }
    ...
    frequencyLocked = core::configManager.conf["frequencyLocked"];
    ...
    // Sauvegarder la fc initiale pour les limites
    initialCenterFreq = frequency;

    // Récupérer bandwidthFactor 
    bandwidthFactor = core::configManager.conf["bandwidthFactor"];

    gui::waterfall.vfoFreqChanged = false;
    gui::waterfall.centerFreqMoved = false;
    gui::waterfall.selectFirstVFO(); // ensure selected VFO is known

    // Now apply the saved zoom and re-center the view
    bw = savedZoom;
    {
    ...
    // Gérer l'autostart (CLI ou configuration)
    autostart = core::args["autostart"].b() || autoStartConf;
    initComplete = true;

    core::moduleManager.doPostInitAll();
}

void MainWindow::centerOnSelection() {
    ImGui::WaterfallVFO* v = nullptr;
    if (gui::waterfall.selectedVFO != "") {
        v = gui::waterfall.vfos[gui::waterfall.selectedVFO];
    }

    // Centrer sur la fréquence sélectionnée (VFO ou fréquence centrale)
    double targetOffset = v ? v->generalOffset : 0.0;
    gui::waterfall.setViewOffset(targetOffset);
}

double MainWindow::clampFrequency(double f) {
    double fc = initialCenterFreq;
    double sr = sigpath::iqFrontEnd.getSampleRate();
    
    double bandMin = fc - ((sr / 2.0) * bandwidthFactor);
    double bandMax = fc + ((sr / 2.0) * bandwidthFactor);
    
    // PRIORITÉ 1 : Limites serveur (ABSOLUES)
    if (serverFreqLimitEnabled && serverFreqMin > 0.0 && serverFreqMax > 0.0) {
        // Limites serveur strictes, elles remplacent fc±sr/2
        double clamped = std::clamp(f, serverFreqMin, serverFreqMax);
        
        if (clamped != f) {
            flog::debug("Frequency {0:.0f} Hz clamped to [{1:.0f} Hz, {2:.0f} Hz] = {3:.0f} Hz (server limits)", 
                        f, serverFreqMin, serverFreqMax, clamped);
        }
        
        return clamped;
    }
    
    // PRIORITÉ 2 : Limites auto fc±sr/2
    if (autoFreqLimitEnabled) {
        double clamped = std::clamp(f, bandMin, bandMax);
        
        if (clamped != f) {
            flog::debug("Frequency {0:.0f} Hz clamped to [{1:.0f} Hz, {2:.0f} Hz] = {3:.0f} Hz (bandwidth limits, ref fc={4:.0f} Hz)", 
                        f, bandMin, bandMax, clamped, fc);
        }
...
void MainWindow::setFrequencyLimits(bool enabled, double minFreq, double maxFreq) {
    serverFreqLimitEnabled = enabled;
    serverFreqMin = minFreq;
    serverFreqMax = maxFreq;

    flog::info("Server frequency limits: enabled={}, min={} Hz, max={} Hz",
               enabled, minFreq, maxFreq);
}

void MainWindow::updateInitialCenterFreq(double fc) {
    initialCenterFreq = fc;
    flog::info("Initial center frequency updated: {} Hz", fc);
}
...
void MainWindow::draw() {
    ...
    // Flags autostart (une seule fois, après 1er rendu)
    static bool firstFrameDrawn = false;
    ...
    // Handle VFO movement
    if (vfo != NULL && !frequencyLocked) {
        if (vfo->centerOffsetChanged) {
            double newFreq = gui::waterfall.getCenterFrequency() + vfo->generalOffset;
            newFreq = clampFrequency(newFreq);
            if (tuningMode == tuner::TUNER_MODE_CENTER) {
                tuner::tune(tuner::TUNER_MODE_CENTER, gui::waterfall.selectedVFO, newFreq);
            }
    ...
    if (gui::waterfall.selectedVFOChanged && !frequencyLocked) {
    ...
    if (gui::freqSelect.frequencyChanged && !frequencyLocked) {
        gui::freqSelect.frequencyChanged = false;
        double nfreq = clampFrequency(gui::freqSelect.frequency);
    ...
    if (gui::waterfall.centerFreqMoved && !frequencyLocked) {
        gui::waterfall.centerFreqMoved = false;
        
        if (autoFreqLimitEnabled && !serverFreqLimitEnabled) {
            flog::debug("Cannot change center frequency: auto limits enabled");
            centerOnSelection();
            return;
        }
        
        // Comportement normal si pas de limites auto (ou si limites serveur)
        double nfreq = clampFrequency(gui::waterfall.getCenterFrequency());
        sigpath::sourceManager.tune(nfreq);
        gui::waterfall.setCenterFrequency(nfreq);
        if (vfo != NULL) {
            gui::freqSelect.setFrequency(nfreq + vfo->generalOffset);
        }
        else {
            gui::freqSelect.setFrequency(nfreq);
        }
        core::configManager.acquire();
        core::configManager.conf["frequency"] = nfreq;
        core::configManager.release(true);
        centerOnSelection();
    }
    ...
    if (playButtonLocked && !tmpPlaySate) {
        style::endDisabled();
    }

    // Frequency lock button
    ImGui::SameLine();
    ImGui::PushID(ImGui::GetID("sdrpp_freq_lock_btn"));
    if (ImGui::ImageButton(frequencyLocked ? icons::LOCK : icons::UNLOCK, btnSize, ImVec2(0, 0), ImVec2(1, 1), 5, ImVec4(0, 0, 0, 0), textCol)) {
        frequencyLocked = !frequencyLocked;
        core::configManager.acquire();
        core::configManager.conf["frequencyLocked"] = frequencyLocked;
        core::configManager.release(true);
    }
    ImGui::PopID();

    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(frequencyLocked ? "Frequency is locked - Click to unlock" : "Click to lock frequency changes");
    }

    ImGui::SameLine();
    float origY = ImGui::GetCursorPosY();

    // ===== CALCUL DYNAMIQUE DE LA LARGEUR DU SLIDER DE VOLUME =====
    float currentX = ImGui::GetCursorPosX();
    float windowWidth = ImGui::GetWindowSize().x;

    // Calculer l'espace nécessaire pour tous les éléments à droite du slider
    float spaceForFreqSelect = 200.0f * style::uiScale;
    float spaceForTunerMode = showTunerMode ? 40.0f * style::uiScale : 0.0f;
    float spaceForSnrMeter = showSnrMeter ? 200.0f * style::uiScale : 0.0f;
    float spaceForLogo = 50.0f * style::uiScale;
    float margin = 20.0f * style::uiScale;

    float totalReserved = spaceForFreqSelect + spaceForTunerMode + spaceForSnrMeter + spaceForLogo + margin;
    float maxSliderWidth = windowWidth - currentX - totalReserved;

    // Limites min/max pour le slider
    float minSliderWidth = showVolumeTaskBar ? 80.0f * style::uiScale : 30.0f * style::uiScale;
    float maxAllowedWidth = 400.0f * style::uiScale;

    volumeTaskBarWidth = std::clamp<float>(maxSliderWidth, minSliderWidth, maxAllowedWidth);

    // Réduire de moitié la largeur du slider du haut
    volumeTaskBarWidth = volumeTaskBarWidth / 2.0f;

    // Sauvegarder la position AVANT le slider
    float posBeforeSlider = ImGui::GetCursorPosX();

    ImGui::SetNextItemWidth(volumeTaskBarWidth);
    sigpath::sinkManager.showVolumeSlider(gui::waterfall.selectedVFO, "##_sdrpp_main_volume_",
                                          volumeTaskBarWidth, 30, 5, true, showVolumeTaskBar);

    // Mesurer la largeur RÉELLE utilisée par le slider
    float posAfterSlider = ImGui::GetCursorPosX();
    float actualSliderWidth = posAfterSlider - posBeforeSlider;

    // Si le slider a débordé, ajuster pour la prochaine frame
    if (actualSliderWidth > volumeTaskBarWidth + 5.0f) { // +5 de tolérance
        // Le slider réel est plus large que prévu, réduire
        float overflow = actualSliderWidth - volumeTaskBarWidth;
        volumeTaskBarWidth = std::max<float>(volumeTaskBarWidth - overflow - 10.0f, minSliderWidth);
    }
    ...
    if (showSnrMeter) {
        ...
        int snrWidth = std::clamp<int>(ImGui::GetWindowSize().x - ImGui::GetCursorPosX() - snrOffset,
                                       100.0f * style::uiScale, 300.0f * style::uiScale);
    ...
    if (!lockWaterfallControls && showMenu) {
        ...
        if (grabbingMenu) {
            ...
            ImGui::GetForegroundDrawList()->AddLine(ImVec2(newWidth, curY), ImVec2(newWidth, winSize.y - 10),
    ...
    if (showMenu) {
        ...
        if (showMenuDebug) {
            if (ImGui::CollapsingHeader("Debug")) {
                ...
                ImGui::Text("Initial Center Frequency: %.0f Hz", initialCenterFreq);
    ...
    // Autostart: une seule fois, après 1 frame complète
    if (!autoStartChecked) {
        if (firstFrameDrawn) {
    ...
    if (!lockWaterfallControls && !frequencyLocked) {
        // Handle arrow keys
    ...
    if (ImGui::VSliderFloat("##_7_", wfSliderSize, &bw, 1.0, 0.0, "")) {
        ...
        centerOnSelection();
        core::configManager.acquire();
        core::configManager.conf["viewBandwidthSlider"] = bw;
        core::configManager.release(true);
    }
...
void MainWindow::setPlayState(bool _playing) {
    ...
    if (_playing) {
        ...
        centerOnSelection(); // re-centrer à la mise en lecture
        playing = true;
