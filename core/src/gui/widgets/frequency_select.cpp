...
void FrequencySelect::draw() {
    ...
    for (int i = hiddenDigits; i < 12; i++) {
        ...
        if ((i + 1) % 3 == 0 && i < 11) {
            commaOffset += commaSz.x;
            window->DrawList->AddText(ImVec2(widgetPos.x + ((i-hiddenDigits) * digitWidth) + commaOffset + textOffset, widgetPos.y),
                                      zeros ? disabledColor : textColor, ".");
        }
    }

    if (!gui::mainWindow.lockWaterfallControls && !gui::mainWindow.frequencyLocked) {
