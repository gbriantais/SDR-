...
public:
    ...
    public:
        ...
        // Nouvelles méthodes pour les contrôles stéréo
    ...
    private:
        ...
        // Nouvelles variables pour les contrôles stéréo
    ...
    void showVolumeSlider(std::string name, std::string prefix, float width, float btnHeight = -1.0f, int btnBorder = 0, bool sameLine = false, bool showSlider=true);
...
private:
    ...
    std::map<std::string, bool> showVolumeSliderMap;
    
    // Nouvelle map pour contrôler l'affichage des contrôles stéréo
    std::map<std::string, bool> showStereoControlsMap;
};