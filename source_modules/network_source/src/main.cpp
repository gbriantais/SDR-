...
enum SampleType {
    SAMPLE_TYPE_INT8,
    SAMPLE_TYPE_INT16,
    SAMPLE_TYPE_INT16_SWAP,
    SAMPLE_TYPE_INT16_ARCALE,
    SAMPLE_TYPE_INT32,
    SAMPLE_TYPE_FLOAT32
};
...
public:
    NetworkSourceModule(std::string name) {
        ...
        sampleTypes.define("Int16 Swap", SAMPLE_TYPE_INT16_SWAP);
        sampleTypes.define("Int16 Arcale", SAMPLE_TYPE_INT16_ARCALE);
...
private:
    ...
    void worker() {
        ...
        // Variable for reconnecting attemps
        ...
        while (!stopRequested) {
            ...
            if (bytes <= 0) {
                // Reconnecting attemps
                reconnecting = true;
                int tries = 0;
                bool reconnected = false;

                while (tries < reconnectAttempts && !stopRequested) {
                    flog::warn("NetworkSourceModule '{0}': Connexion perdue, tentative de reconnexion ({1}/{2})...", name, tries + 1, reconnectAttempts);
                    std::this_thread::sleep_for(std::chrono::milliseconds(reconnectDelayMs));
                    if (stopRequested) break;
                    try {
                        if (proto == PROTOCOL_TCP_CLIENT) {
                            ...
                            // Server not supported here
                            break;
                        }
                        reconnected = true;
                        flog::info("NetworkSourceModule '{0}' : Reconnexion réussie!", name);
                        break;
                    }
                    catch (const std::exception& e) {
                        flog::error("NetworkSourceModule '{0}' : Echec de reconnexion!", name);
                    }
                    tries++;
                }
                reconnecting = false;
                if (!reconnected || stopRequested) {
                    flog::error("NetworkSourceModule '{0}' : Echec de reconnexion après {1} tentatives ou arrêt demandé.", name, reconnectAttempts);
                    break;
                }
                else {
                    continue;
            ...
            case SAMPLE_TYPE_INT16:
                ...
                break;
            case SAMPLE_TYPE_INT16_SWAP: {
                std::vector<int16_t> swapped(count * 2);
                int16_t* src = (int16_t*)buffer;
                for (int i = 0; i < count * 2; ++i) {
                    uint16_t v = ((uint8_t*)src)[2*i] | (((uint8_t*)src)[2*i + 1] << 8);
                    swapped[i] = (v >> 8) | (v << 8);
                }
                volk_16i_s32f_convert_32f((float*)stream.writeBuf, swapped.data(), 32768.0f, count * 2);
                break;
            }
            case SAMPLE_TYPE_INT16_ARCALE: {
                int skipBytes = 10;
                int16_t* srcArcale = (int16_t*)((uint8_t*)buffer + skipBytes);
                int countSwap = (bytes - skipBytes) / sizeof(int16_t);
                std::vector<int16_t> swappedArcale(countSwap);
                for (int i = 0; i < countSwap; ++i) {
                    uint16_t v = ((uint8_t*)srcArcale)[2*i] | (((uint8_t*)srcArcale)[2*i + 1] << 8);
                    swappedArcale[i] = (v >> 8) | (v << 8);
                }
                volk_16i_s32f_convert_32f((float*)stream.writeBuf, swappedArcale.data(), 32768.0f, countSwap);
                count = countSwap / 2; // car count = nb couple IQ
                break;
            }
            ...
            if (!stream.swap(count) || stopRequested) { 
                break; 
            }
        }
